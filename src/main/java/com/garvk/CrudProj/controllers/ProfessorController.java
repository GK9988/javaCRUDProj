...
public class ProfessorController {
    ...
    public ResponseEntity<?> updateMarks(@PathVariable Integer id, @RequestBody Map<String, Object> requestBody){
        int lUid = (int) requestBody.get("uid");
        int lMarks = (int) requestBody.get("marks");

        try{
            professorService.updateMarks(lUid, lMarks, id);
        } catch(Exception e){
            return new ResponseEntity<>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }

        return new ResponseEntity<>("Marks Updated", HttpStatus.CREATED);
